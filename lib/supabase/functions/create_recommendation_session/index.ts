...
function log(ctx: { reqId: string; userId?: string | null }, msg: string, extra?: unknown) {
  ...
  const base = `[create_recommendation_session][${ts}][req:${ctx.reqId}]` + (ctx.userId ? `[user:${ctx.userId}]` : "[user:unknown]");
...
async function fetchAndStoreWatchProviders(
  ...
  try {
    ...
    if (!res.ok) {
      log(ctx, "TMDB watch providers fetch failed", { tmdbId, tmdbType, status: res.status });
      return [];
    }
    const json = await res.json();
    const regionData = json?.results?.[region] ?? json?.results?.["US"] ?? null;
    if (!regionData) {
      log(ctx, "No watch providers for region", { tmdbId, tmdbType, region });
      return [];
    }
    const watchLink = regionData.link ?? null;
    if (watchLink) {
      await supabaseAdmin.from("media_titles").update({ watch_provider_link: watchLink, updated_at: nowIso() }).eq("id", titleId);
    }
    const flatrateProviders: any[] = regionData.flatrate ?? [];
    const providers: WatchProvider[] = [];
    for (const p of flatrateProviders) {
      const providerId = p.provider_id;
      const providerName = p.provider_name ?? "Unknown";
      const logoPath = p.logo_path ? `https://image.tmdb.org/t/p/w92${p.logo_path}` : null;
      const displayPriority = p.display_priority ?? 100;
      const { data: existing } = await supabaseAdmin.from("streaming_providers").select("id").eq("tmdb_provider_id", providerId).maybeSingle();
      let providerDbId: string;
      if (existing) {
        providerDbId = existing.id;
        await supabaseAdmin.from("streaming_providers").update({ name: providerName, logo_url: logoPath, display_priority: displayPriority, updated_at: nowIso() }).eq("id", providerDbId);
      } else {
        const { data: inserted, error: insertErr } = await supabaseAdmin.from("streaming_providers").insert({ tmdb_provider_id: providerId, name: providerName, logo_url: logoPath, display_priority: displayPriority, created_at: nowIso(), updated_at: nowIso() }).select("id").single();
        if (insertErr) {
          log(ctx, "Failed to insert streaming provider", { providerId, error: insertErr });
          continue;
        }
        providerDbId = inserted.id;
      }
      const { error: availErr } = await supabaseAdmin.from("title_streaming_availability").upsert({ title_id: titleId, provider_id: providerDbId, region, availability_type: "flatrate", watch_link: watchLink, updated_at: nowIso() }, { onConflict: "title_id,provider_id,region,availability_type" });
      if (availErr) {
        log(ctx, "Failed to upsert streaming availability", { titleId, providerDbId, error: availErr });
      }
      providers.push({ provider_id: providerId, name: providerName, logo_url: logoPath ?? "", display_priority: displayPriority });
    }
    log(ctx, "Stored watch providers", { titleId, region, count: providers.length });
    return providers;
  } catch (err) {
    log(ctx, "Error fetching watch providers", { tmdbId, tmdbType, error: String(err) });
    return [];
  }
}
...
async function getOrCreateMediaTitle(
  ...
  if (existing && !needsEnrich) {
    if (fetchProviders) {
      const { count } = await supabaseAdmin.from("title_streaming_availability").select("id", { count: "exact", head: true }).eq("title_id", existing.id).eq("region", region);
      if (!count || count === 0) {
        fetchAndStoreWatchProviders(ctx, supabaseAdmin, tmdbId, tmdbType, existing.id, region);
      }
    }
    return existing;
  }
  ...
  if (existing) {
    ...
    if (fetchProviders && updated) {
      fetchAndStoreWatchProviders(ctx, supabaseAdmin, tmdbId, tmdbType, updated.id, region);
    }
    return updated;
  }
  ...
  if (fetchProviders && inserted) {
    fetchAndStoreWatchProviders(ctx, supabaseAdmin, tmdbId, tmdbType, inserted.id, region);
  }
  return inserted;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile_id || !session_type) {
      return new Response(JSON.stringify({ error: "Missing profile_id or session_type" }), { status: 400, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
    }
    ...
    const { data: appUser } = await supabaseAdmin.from("app_users").select("region").eq("id", userId).maybeSingle();
    const userRegion = appUser?.region ?? DEFAULT_REGION;
    log(ctx, "User region for watch providers", { userRegion });
    ...
    async function processCandidates(items: any[]) {
      for (const it of items) {
        ...
        const mediaRow = await getOrCreateMediaTitle(ctx, supabaseAdmin, hit.tmdb_id, hit.tmdb_type, title, true, userRegion);
    ...
    return new Response(JSON.stringify({ id: session.id, profile_id, session_type, mood_input: { ...(mood_input ?? {}), content_types }, created_at: session.created_at, cards }), {
      status: 200,
